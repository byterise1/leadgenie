...
export function Navbar() {
  ...
  return (
    <header ref={ref} className="sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 shadow-sm">
      <div className="container flex items-center justify-between h-[64px]">
        ...
        <Link href="/" onClick={close} className="flex items-center gap-2 shrink-0">
          <Logo size={58} textSize="text-[16px]" />
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm font-semibold">
          {([['products','Products'],['usecases','Solutions'],['resources','Resources']] as [DropKey,string][]).map(([k,l]) => (
            <button key={k} onClick={() => setDrop(p => p===k ? null : k)}
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl transition-all duration-150
                ${drop===k ? 'text-blue-600 bg-blue-50 dark:bg-blue-950/50' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
              {l}
          ...
          <Link href="/pricing" onClick={close}
            className="px-4 py-2.5 rounded-xl text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-150">
            Pricing
          </Link>
        </nav>
          ...
          {user ? (
            <>
              <Link href="/dashboard" onClick={close}
                className="px-4 py-2.5 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
                Dashboard
              </Link>
              <button
                onClick={handleSignOut}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              >
        ...
        <div className="md:hidden flex items-center gap-1">
          <ThemeToggle />
          <button onClick={() => setMob(v=>!v)} className="p-2 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="menu">
          {mob
      ...
      {/* -- Products dropdown -- */}
      {drop === 'products' && (
        <div className="absolute inset-x-0 top-[64px] bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-2xl z-50">
          <div className="container py-8">
            <div className="grid grid-cols-[1fr_1fr_280px] gap-8">
              {products.map(col => (
                <div key={col.col}>
                  <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.18em] mb-4">{col.col}</p>
                  <div className="space-y-1">
                    {col.items.map(item => (
                      <Link key={item.label} href={item.href} onClick={close}
                        className="flex items-center gap-3 p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 group transition-colors">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-gray-700 dark:group-hover:text-gray-200">{item.label}</span>
              ...
              <div className="border-l border-gray-100 dark:border-gray-800 pl-8 flex flex-col justify-between">
                <div>
                  <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.18em] mb-4">Get Started</p>
                  <div className="rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm">
                    ...
                    <div className="p-4 bg-white dark:bg-gray-800">
                      <p className="text-sm font-bold text-gray-900 dark:text-white">Start free today</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 mb-3">No credit card — 5-min setup</p>
                      <Link href="/signup" onClick={close}
                ...
                <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-4 leading-relaxed">
                  Join <strong className="text-gray-600 dark:text-gray-300">8,500+</strong> teams using Leads Genie to book more meetings.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="container py-8">
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 max-w-3xl">
              {useCases.map((item, idx) => (
                <Link key={item.label} href={item.href} onClick={close}
                  className="flex items-start gap-3 p-4 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 group transition-colors border border-transparent hover:border-gray-100 dark:hover:border-gray-700">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{item.label}</p>
      ...
      {/* -- Mobile menu -- */}
      {mob && (
        <div className="md:hidden bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 px-4 py-4 max-h-[80vh] overflow-y-auto shadow-xl">
          {products.map(col => (
            <div key={col.col} className="mb-5">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400 dark:text-gray-500 px-2 mb-2">{col.col}</p>
              {col.items.map(item => (
                <Link key={item.label} href={item.href} onClick={close}
                  ...
                  {item.badge && <span className="ml-auto text-[9px] font-black bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 rounded-full px-1.5 py-0.5">{item.badge}</span>}
                </Link>
              ))}
            </div>
          ))}
          <div className="mb-4">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400 dark:text-gray-500 px-2 mb-2">Solutions</p>
            {useCases.map((item, idx) => (
              <Link key={item.label} href={item.href} onClick={close}
                className="flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
          ...
          <div className="mb-4">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400 dark:text-gray-500 px-2 mb-2">Resources</p>
            {resources.map((item, idx) => (
              <Link key={item.label} href={item.href} onClick={close}
                className="flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
          ...
          <div className="border-t border-gray-100 dark:border-gray-800 pt-4 flex flex-col gap-2">
            {user ? (
              <>
                ...
                <button onClick={handleSignOut} className="text-center py-2.5 text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white transition-colors">Sign out</button>
              </>
            ) : (
              <>
                <Link href="/login" onClick={close} className="text-center py-2.5 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:text-gray-900 dark:hover:text-white">Login</Link>
