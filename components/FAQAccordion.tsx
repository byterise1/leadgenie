    ...
    <section className="py-24 bg-white">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto text-center mb-14">
          <span className="inline-block text-xs font-bold bg-blue-50 text-blue-600 rounded-full px-3 py-1 mb-4 uppercase tracking-widest">FAQ</span>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight leading-tight mb-4">Frequently Asked Questions</h2>
          <p className="text-gray-500 text-base leading-relaxed">Everything you need to know about Leads Genie. Can't find an answer? <a href="/contact" className="text-blue-600 font-semibold hover:underline">Contact us.</a></p>
        </div>

        <div className="max-w-2xl mx-auto divide-y divide-gray-100 border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
          {faqs.map((faq, i) => {
            const isOpen = open === i;
            return (
              <div key={i} className={`transition-colors ${isOpen ? 'bg-blue-50/40' : 'bg-white hover:bg-gray-50/60'}`}>
                <button
                  ...
                  <span className={`text-sm font-semibold leading-snug transition-colors ${isOpen ? 'text-blue-700' : 'text-gray-900 group-hover:text-blue-600'}`}>
                    {faq.q}
                  </span>
                  <span className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center transition-all ${isOpen ? 'bg-blue-600 text-white rotate-180' : 'bg-gray-100 text-gray-500'}`}>
