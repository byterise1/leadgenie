...
//   - Gated on THIS campaign's own is_test_campaign flag, set explicitly at
//     creation time (independent of delay timing — every campaign, test or
//     not, uses real day-based delays since TEST_MODE_FAST_FOLLOWUPS was
//     retired 2026-07-11) — a real production campaign always gets a 403
//     here, full stop, no override.
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: campaign } = await supabaseAdmin
    .from('campaigns')
    .select('id, name, status, is_test_campaign')
    .eq('id', campaignId)
    .eq('user_id', user.id)
    .single();
  ...
  if (!campaign.is_test_campaign) {
    return NextResponse.json({
      error: 'This is a test-only action, only available on campaigns marked as a test campaign at creation. Real campaigns always wait their real configured delay.',
    }, { status: 403 });
  }
