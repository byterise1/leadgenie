    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[92vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
          <h2 className="text-base font-bold text-gray-900 dark:text-white">{isEdit ? 'Edit Template' : 'Create Template'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-500 mb-1.5">Template Name</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. My Cold Email"
                className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"/>
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-500 mb-1.5">Category</label>
              <select value={category} onChange={e => setCategory(e.target.value)}
                className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500 transition">
          ...
          {/* Variable helper */}
          <div>
            <p className="text-xs font-bold text-gray-500 dark:text-gray-500 mb-1.5">
              Variables <span className="font-normal text-gray-400 dark:text-gray-500">— click in subject or body first, then insert</span>
            </p>
            <div className="flex flex-wrap gap-1.5">
              {VARIABLES.map(v => (
                <button key={v} type="button" onClick={() => insertVar(v)}
                  className="text-xs font-mono font-bold bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 border border-blue-100 rounded-lg px-2.5 py-1 hover:bg-blue-100 transition-colors">
                  {v}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1 w-fit">
            {tabs.map(t => (
              <button key={t.id} type="button" onClick={() => setTab(t.id)}
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${tab === t.id ? 'bg-white text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-500 hover:text-gray-700'}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {tab === 'edit' && (
            <>
              {/* Subject */}
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-gray-500 mb-1.5">Subject Line</label>
              ...
              <div>
                <div className="flex items-center gap-1 mb-1.5">
                  <label className="text-xs font-bold text-gray-500 dark:text-gray-500 flex-1">Email Body</label>
                  <div className="flex gap-0.5 bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5">
                    {[
                      ...
                      <button key={f.label} type="button" title={f.title} onClick={() => { setActiveField('body'); applyFormat(f.o, f.c); }}
                        className="w-7 h-7 flex items-center justify-center rounded-md text-xs font-bold text-gray-500 dark:text-gray-500 hover:bg-white hover:text-gray-900 dark:text-white hover:shadow-sm transition-all">
                    ...
                    <button type="button" title="Unordered list" onClick={() => { setActiveField('body'); applyFormat('<ul>\n  <li>', '</li>\n</ul>'); }}
                      ...
                      <svg className="w-3.5 h-3.5 text-gray-500 dark:text-gray-500 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16"/></svg>
                    </button>
                    <button type="button" title="Button" onClick={openBtnDialog}
                      className={`px-2 h-7 flex items-center justify-center rounded-md text-[10px] font-bold transition-all ${btnDialog ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 dark:text-gray-500 hover:bg-white hover:shadow-sm'}`}>
                      BTN
                    </button>
                  </div>
                </div>
                {linkDialog && (
                  <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-3 py-2">
                    ...
                    <input ref={linkUrlRef} value={linkUrl} onChange={e => setLinkUrl(e.target.value)}
                      ...
                      className="flex-1 text-sm bg-transparent outline-none text-gray-700 dark:text-gray-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 dark:text-gray-500 min-w-0"/>
                    <label className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-500 shrink-0 cursor-pointer select-none">
                    ...
                    <button type="button" onClick={() => setLinkDialog(false)}
                      className="w-6 h-6 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:bg-gray-800 rounded-lg transition-all shrink-0">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-2.5 space-y-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <input value={btnText} onChange={e => setBtnText(e.target.value)}
                        ...
                        className="flex-1 text-sm bg-white border border-blue-200 rounded-lg px-2.5 py-1.5 outline-none text-gray-700 dark:text-gray-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 dark:text-gray-500 focus:ring-2 focus:ring-blue-400"/>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <button type="button" onClick={() => setBtnDialog(false)}
                        className="w-7 h-7 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 hover:bg-white rounded-lg transition-all shrink-0">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12"/></svg>
                      </button>
                    </div>
                ...
                <textarea ref={bodyRef} value={body} onChange={e => setBody(e.target.value)}
                  ...
                  rows={9}
                  className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none leading-relaxed font-mono"/>
              </div>
              ...
              <div className="border-t border-dashed border-gray-200 pt-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-gray-700 dark:text-gray-200">Include unsubscribe footer</p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">Adds an opt-out link at the bottom of the email</p>
                  </div>
                ...
                {includeUnsub && (
                  <>
                    <textarea value={unsubText} onChange={e => setUnsubText(e.target.value)} rows={2}
                      className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-xs text-gray-500 dark:text-gray-500 outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none font-mono"/>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">Keep <span className="font-mono">{'{{unsubscribe_link}}'}</span> in the text — it gets replaced with a real opt-out URL on send.</p>
                  </>
                )}
              </div>
            </>
          ...
          {tab === 'preview' && (
            <div className="bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-800 rounded-2xl p-5 space-y-3">
              <div>
                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Subject</p>
                <p className="text-sm font-medium text-gray-900 dark:text-white">{subject || <span className="text-gray-300 dark:text-gray-600 font-normal">No subject yet</span>}</p>
              </div>
              <div className="border-t border-gray-100 dark:border-gray-800 pt-3">
                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">Body</p>
                {body ? (
                  <iframe
                  ...
                  <p className="text-sm text-gray-300 dark:text-gray-600">No body yet</p>
                )}
              </div>
            </div>
          )}
          ...
          {tab === 'html' && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-gray-500 dark:text-gray-500 dark:text-gray-500">HTML Body <span className="font-normal text-gray-400 dark:text-gray-500">— edit raw HTML directly</span></p>
              ...
              <textarea value={body} onChange={e => setBody(e.target.value)} rows={12}
                ...
                className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-3 text-xs text-gray-600 dark:text-gray-300 font-mono outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-gray-50 dark:bg-gray-800 resize-none"/>
              <div className="border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden">
                <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider px-3 py-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-800">Rendered Preview</p>
                <iframe
                  srcDoc={bodyToHtml(body, unsubText, includeUnsub)}
        ...
        <div className="flex gap-3 px-4 sm:px-6 py-4 border-t border-gray-100 dark:border-gray-800 shrink-0">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-200 text-gray-600 dark:text-gray-300 font-semibold text-sm rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            Cancel
          </button>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
        <h2 className="text-base font-bold text-gray-900 dark:text-white mb-1">Use this template</h2>
        <p className="text-sm text-gray-400 dark:text-gray-500 mb-5">Where do you want to use <span className="font-semibold text-gray-700 dark:text-gray-200">"{template.name}"</span>?</p>
        <div className="space-y-2 mb-5">
          <button onClick={() => {
            ...
            className="w-full flex items-center gap-3 p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all text-left">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-white">Start New Campaign</p>
              <p className="text-xs text-gray-400 dark:text-gray-500">Use this template in a new campaign</p>
            </div>
          ...
          <button onClick={() => { onDuplicate(template); onClose(); }}
            className="w-full flex items-center gap-3 p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:border-emerald-300 hover:bg-emerald-50 transition-all text-left">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-white">Duplicate & Customise</p>
              <p className="text-xs text-gray-400 dark:text-gray-500">Copy to My Templates and edit freely</p>
            </div>
            <svg className="w-4 h-4 text-gray-300 dark:text-gray-600 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
          </button>
        </div>
        <button onClick={onClose} className="w-full py-2.5 text-sm text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-300 transition-colors">Cancel</button>
      </div>
    </div>
  );
}
    ...
    <main className="flex-1 p-4 sm:p-6 overflow-x-hidden w-full max-w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Ready-to-use templates — edit, create, or launch in campaigns.</p>
        </div>
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search templates..."
          className="w-full pl-9 pr-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"/>
      </div>

      <div className="flex flex-wrap gap-2 mb-6">
        {allCats.map(c => (
          <button key={c} onClick={() => setFilterCat(c)}
            className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${filterCat === c ? 'bg-blue-600 text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600 dark:text-gray-300 hover:border-blue-300 hover:text-blue-600'}`}>
            {c}
          </button>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 min-w-0">
        {filtered.map(t => (
          <div key={t.id} className="bg-white rounded-2xl border border-gray-100 dark:border-gray-800 p-5 hover:shadow-md hover:border-blue-100 transition-all flex flex-col min-w-0 overflow-hidden">
            <div className="flex items-start justify-between mb-3">
              ...
              {t.builtIn && <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 rounded-full px-2 py-0.5 shrink-0">Built-in</span>}
            </div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-1 truncate">{t.name}</h3>
            <p className="text-xs text-gray-500 dark:text-gray-500 font-medium mb-1 truncate">Subject: {t.subject}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 leading-relaxed flex-1 line-clamp-2 mb-4">
              {t.body.split('\n').filter(l => l.trim())[0]}
            </p>
            <div className="flex items-center gap-3 mb-4 pt-3 border-t border-gray-100 dark:border-gray-800">
              <div className="text-center"><p className="text-sm font-bold text-gray-900 dark:text-white">{t.openRate}</p><p className="text-[10px] text-gray-400 dark:text-gray-500">Open</p></div>
              <div className="w-px h-6 bg-gray-100 dark:bg-gray-800"/>
              <div className="text-center"><p className="text-sm font-bold text-gray-900 dark:text-white">{t.replyRate}</p><p className="text-[10px] text-gray-400 dark:text-gray-500">Reply</p></div>
              <div className="w-px h-6 bg-gray-100 dark:bg-gray-800"/>
              <div className="text-center"><p className="text-sm font-bold text-gray-900 dark:text-white">{t.uses > 999 ? `${(t.uses / 1000).toFixed(1)}k` : t.uses || '—'}</p><p className="text-[10px] text-gray-400 dark:text-gray-500">Uses</p></div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setEditTarget(t)} className="flex-1 text-xs font-bold text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-xl py-2 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Edit</button>
              ...
              {!t.builtIn && t.dbId && (
                <button onClick={() => setDeleteId(t.dbId!)} className="px-2.5 text-gray-300 dark:text-gray-600 hover:text-red-400 border border-gray-100 dark:border-gray-800 rounded-xl transition-colors">
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center py-20 text-center">
          <p className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">No templates found</p>
