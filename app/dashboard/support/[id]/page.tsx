...
export default function UserTicketPage() {
  ...
  if (loading) {
    return (
      <main className="flex-1 p-6 max-w-3xl">
        <div className="h-4 w-24 bg-gray-100 dark:bg-gray-800 rounded animate-pulse mb-6"/>
  ...
  if (!ticket) {
    return (
      <main className="flex-1 p-6 max-w-3xl">
        <button onClick={() => router.push('/dashboard/support')} className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-white mb-4 transition-colors">
  ...
  return (
    <main className="flex-1 p-6 max-w-3xl space-y-6">
      <button onClick={() => router.push('/dashboard/support')}
        className="flex items-center gap-1.5 text-sm text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-white transition-colors">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 flex-wrap mt-2 text-xs">
          ...
          <span className="text-gray-400 dark:text-gray-500 capitalize">{ticket.category}</span>
          <span className="text-gray-200 dark:text-gray-600">·</span>
          <span className="text-gray-400 dark:text-gray-500">Opened {fmtDate(ticket.created_at)}</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {thread.map((msg, i) => (
          msg.role === 'user' ? (
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {msg.attachments.map((a, j) => (
                      <a key={j} href={a.url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1">
            ...
            <div key={i} className="flex gap-3 items-start flex-row-reverse">
              <div className="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-950/50 flex items-center justify-center shrink-0 text-sm font-bold text-blue-600 dark:text-blue-400">
                LG
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1.5 justify-end">
                  <span className="text-xs text-gray-400 dark:text-gray-500">{fmtDate(msg.ts)}</span>
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Leads Genie Support</span>
                </div>
        ...
        {!ticket.admin_reply && (
          <div className="flex items-center gap-3 rounded-2xl bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 px-5 py-4">
            <div className="w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-950/50 flex items-center justify-center shrink-0">
              <svg className="w-4 h-4 text-blue-400 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Awaiting response</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Our team usually replies within 24 hours. You&apos;ll get a notification when we do.</p>
            </div>
          </div>
        )}
      </div>
      ...
      {ticket.status !== 'closed' ? (
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <button onClick={() => fileRef.current?.click()} type="button"
                className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-white transition-colors">
        ...
        <div className="text-center py-4 border-t border-gray-100 dark:border-gray-800">
          <p className="text-xs text-gray-400 dark:text-gray-500">This ticket is closed. Open a new ticket if you need further help.</p>
        </div>
      )}
    </main>
  );
