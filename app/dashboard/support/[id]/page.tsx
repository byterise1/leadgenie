    ...
    <main className="flex-1 p-6 max-w-3xl space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Open a ticket and we&apos;ll get back to you.</p>
        </div>
      ...
      {showNew && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 space-y-4">
          <h2 className="text-sm font-bold text-gray-900 dark:text-white">Open a New Ticket</h2>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value)}
                className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-300 transition-colors">
            ...
            {files.length > 0 && (
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {files.map((f, i) => (
                  <span key={i} className="flex items-center gap-1 text-[10px] font-semibold bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-lg px-2 py-0.5">
                    {f.name}
                    <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} className="text-gray-400 dark:text-gray-500 hover:text-red-500 ml-0.5">×</button>
                  </span>
                ))}
              </div>
            )}
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => setShowNew(false)} className="px-5 py-2 text-sm font-semibold text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 transition-colors">
              Cancel
            </button>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <div className="w-12 h-12 bg-gray-50 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-3">
              <svg className="w-6 h-6 text-gray-300 dark:text-gray-600 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
              </svg>
            </div>
            <p className="text-sm font-semibold text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500">No tickets yet</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Open your first ticket above — we usually respond within 24 hours.</p>
          </div>
        ) : (
          <div className="divide-y divide-gray-50 dark:divide-gray-800">
            {tickets.map(t => (
              <div key={t.id} onClick={() => router.push(`/dashboard/support/${t.id}`)}
                className="px-5 py-4 flex items-center gap-4 hover:bg-gray-50 dark:bg-gray-800 cursor-pointer transition-colors group">
