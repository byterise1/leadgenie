...
function SmtpForm({ onBack, onConnect, connecting }: { onBack: () => void; onConnect: (email: string, extra?: Record<string, string>) => void; connecting?: boolean }) {
  ...
  // Auto-derive IMAP host from SMTP host when user types it
  const setHost = (v: string) => {
    setForm(f => ({
      ...f,
      host: v,
      imapHost: f.imapHost || v.replace(/^smtp\./i, 'imap.'),
    }));
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {/* IMAP — always shown, required for reply detection */}
...
export default function EmailAccountsPage() {
  ...
  useEffect(() => { setAddError(''); if (!step) { setJoinSharedNetwork(true); } }, [step]);
  ...
  const addAccount = async (type: Account['type'], email: string, extra?: Record<string, string>) => {
    ...
    const res = await fetch('/api/email-accounts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type, email, join_shared_network: joinSharedNetwork, ...extra }),
    });
    const data = await res.json();
              ...
              {step === 'gmail-oauth' && (
                <div className="text-center py-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <a href={`/api/email-accounts/oauth/google?join_shared_network=${joinSharedNetwork ? '1' : '0'}`} target="_blank" rel="noopener noreferrer"
