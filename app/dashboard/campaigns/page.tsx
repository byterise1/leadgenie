...
const STATUS_BADGE: Record<string, string> = {
  active: 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border-emerald-200',
  paused: 'bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-400 border-amber-200',
  completed: 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 border-blue-200',
    ...
    <main className="flex-1 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Manage all your cold email campaigns.</p>
        </div>
      ...
      {/* Status tabs */}
      <div className="flex gap-1 mb-6 bg-gray-100 dark:bg-gray-800 rounded-xl p-1 max-w-full overflow-x-auto">
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all whitespace-nowrap shrink-0 ${tab === t ? 'bg-white text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-500 hover:text-gray-700'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-gray-100 dark:border-gray-800 flex items-center overflow-hidden">
              <div className="w-1.5 self-stretch bg-gray-100 dark:bg-gray-800" />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 dark:border-gray-800 py-20 text-center">
            <div className="flex flex-col items-center">
              <div className="w-12 h-12 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-800 flex items-center justify-center mb-4">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-5">Launch your first campaign to start booking meetings.</p>
            ...
            <div key={c.id}
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0 px-4 py-3.5">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {c.list_name && (
                      <span className="text-[11px] text-gray-400 dark:text-gray-500 truncate">· {c.list_name}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="hidden lg:flex items-center gap-10 px-6 shrink-0">
                <div className="text-center min-w-[40px]">
                  <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5">Sent</p>
                  <p className={`text-sm font-bold ${isDraft ? 'text-gray-300' : 'text-gray-900'}`}>{c.total_sent ?? 0}</p>
                </div>
                <div className="text-center min-w-[40px]">
                  <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5">Opens</p>
                  <p className={`text-sm font-bold ${openRate !== '—' ? 'text-gray-900' : 'text-gray-300'}`}>{openRate}</p>
                </div>
                <div className="text-center min-w-[40px]">
                  <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-0.5">Replies</p>
              ...
              <div className="flex items-center gap-2 px-4 shrink-0">
                {/* Always-visible: View */}
                <Link href={`/dashboard/campaigns/${c.id}`}
                  className="text-xs font-bold px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-800 border border-gray-200 transition-colors whitespace-nowrap">
                  View
                </Link>
                ...
                {c.status === 'draft' && (
                  <button onClick={e => startCampaign(e, c)} disabled={startingId === c.id}
                    className="hidden sm:inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border-emerald-200 hover:bg-emerald-100 disabled:opacity-50 transition-colors whitespace-nowrap">
                    {startingId === c.id
                ...
                {c.status === 'active' && (
                  <button onClick={e => pauseCampaign(e, c)}
                    className="hidden sm:inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-400 border-amber-200 hover:bg-amber-100 transition-colors whitespace-nowrap">
                ...
                <div ref={openMenuId === c.id ? menuRef : null} className="relative">
                  <button
                    ...
                    className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 transition-colors">
                  ...
                  {openMenuId === c.id && (
                    <div className="absolute right-0 top-9 z-50 bg-white border border-gray-100 dark:border-gray-800 rounded-xl shadow-xl min-w-[168px] py-1">
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2.5 px-3.5 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
