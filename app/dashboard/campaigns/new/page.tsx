...
// Mock data — in production comes from real state/db
const MOCK_ACCOUNTS = [
  { id: 1, email: 'john@company.com', type: 'Gmail OAuth', limit: 100 },
  { id: 2, email: 'outreach@startup.io', type: 'Custom SMTP', limit: 200 },
  { id: 3, email: 'sales@agency.co', type: 'Gmail App Password', limit: 50 },
];
...
type EmailStep = { subject: string; body: string; delay: number; templateId: number | null };

const DEFAULT_EMAIL: EmailStep = { subject: '', body: '', delay: 3, templateId: null };
...
export default function NewCampaignPage() {
  ...
  const [perCampaignLimit, setPerCampaignLimit] = useState<number | ''>('');
  ...
  const totalDailyLimit = perCampaignLimit
    ? Number(perCampaignLimit)
    : activeAccountCount
      ? (allAccounts ? MOCK_ACCOUNTS : MOCK_ACCOUNTS.filter(a => selectedAccounts.includes(a.id))).reduce((s, a) => s + a.limit, 0)
      : 0;
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-gray-400">Emails rotate across selected accounts</span>
              </div>
              {MOCK_ACCOUNTS.length === 0 ? (
                <div className="border border-dashed border-gray-200 rounded-xl p-4 text-center">
                  <p className="text-sm text-gray-400 mb-2">No email accounts connected</p>
                  <Link href="/dashboard/email-accounts" className="text-xs font-bold text-blue-600 hover:underline">+ Connect an account first →</Link>
                </div>
              ) : (
                <div className="border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  <label className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors">
                    <input type="checkbox" checked={allAccounts} onChange={toggleAllAccounts}
                      className="w-4 h-4 rounded accent-blue-600"/>
                    <span className="text-sm font-semibold text-gray-700">All accounts ({MOCK_ACCOUNTS.length})</span>
                    <span className="text-xs text-gray-400 ml-auto">Total: {MOCK_ACCOUNTS.reduce((s, a) => s + a.limit, 0)}/day</span>
                  </label>
                  {MOCK_ACCOUNTS.map(acc => (
                    <label key={acc.id} className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 last:border-0 cursor-pointer hover:bg-gray-50 transition-colors">
                      <input type="checkbox"
                        checked={allAccounts || selectedAccounts.includes(acc.id)}
                      ...
                      <span className="text-xs font-semibold text-gray-500">{acc.limit}/day</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            {/* Per-campaign daily cap (optional override) */}
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-sm font-semibold text-gray-700">Campaign Daily Cap</label>
                <span className="text-xs text-gray-400">Optional — overrides account limits</span>
              </div>
              <div className="flex items-center gap-3">
                <input type="number" value={perCampaignLimit} onChange={e => setPerCampaignLimit(e.target.value ? Number(e.target.value) : '')}
                  placeholder={`Auto (${totalDailyLimit || '—'}/day from accounts)`}
                  className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"/>
                <span className="text-xs text-gray-400 shrink-0">emails/day</span>
              </div>
              {activeAccountCount > 0 && (
                <p className="text-xs text-gray-400 mt-1.5">
                  {activeAccountCount} account{activeAccountCount > 1 ? 's' : ''} selected · auto cap = <span className="font-semibold text-gray-600">{totalDailyLimit}/day</span>
                  {activeAccountCount > 1 && <span className="text-gray-400"> (rotated evenly)</span>}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            {emails.map((email, idx) => (
              <div key={idx} className="bg-white rounded-2xl border border-gray-100 p-6">
                  ...
                  <div className="flex items-center gap-3">
                    {idx > 0 && (
                      <div className="flex items-center gap-1.5 text-xs text-gray-400">
                        ...
                        <select value={email.delay} onChange={e => setEmails(em => em.map((x, i) => i === idx ? { ...x, delay: Number(e.target.value) } : x))}
                    ...
                    {idx > 0 && (
                      <button onClick={() => setEmails(em => em.filter((_, i) => i !== idx))}
                ...
                <button onClick={() => setTemplatePickerIdx(idx)}
                  ...
                  {email.templateId
                    ? `Template: ${MOCK_TEMPLATES.find(t => t.id === email.templateId)?.name} — click to change`
                    : 'Pick from template library (optional)'}
                </button>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: 'To', val: toHour, set: setToHour, opts: ['4:00 PM','5:00 PM','6:00 PM','7:00 PM','8:00 PM','9:00 PM'] },
                ].map(f => (
                  <div key={f.label}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
            ...
            <div className="divide-y divide-gray-100">
              {[
                ...
                { label: 'Daily Limit', value: totalDailyLimit ? `${totalDailyLimit} emails/day` : '—' },
            ...
            {activeAccountCount === 0 && (
              <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 text-xs text-amber-700 font-medium">
                ⚠️ Select at least one email account to launch this campaign.
              </div>
            )}
              ...
              {MOCK_TEMPLATES.map(t => (
                <button key={t.id} onClick={() => applyTemplate(templatePickerIdx, t.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 truncate mt-0.5">{t.body.split('\n').filter(l => l.trim())[0]}</p>
                  </div>
