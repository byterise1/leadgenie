...
import DeliverabilityCheckPanel from '@/components/DeliverabilityCheckPanel';
...
type EmailStep = { subject: string; body: string; delay: number; templateId: string | null; includeUnsub: boolean; includeTracking: boolean; threadMode: 'reply' | 'new_thread'; abEnabled: boolean; abVariants: AbVariant[] };
const DEFAULT_EMAIL: EmailStep = { subject: '', body: '', delay: 0, templateId: null, includeUnsub: false, includeTracking: true, threadMode: 'new_thread', abEnabled: false, abVariants: [{ subject: '', body: '' }] };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (prefill) {
        ...
        setEmails([{ subject: t.subject || '', body: t.body || '', delay: 0, templateId: t.templateId || null, includeUnsub: false, includeTracking: true, threadMode: 'new_thread', abEnabled: false, abVariants: [{ subject: '', body: '' }] }]);
            ...
            {emails.map((email, idx) => (
              <div key={idx} className="bg-white rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
                ...
                <div className="space-y-3">
                  ...
                  {/* Rotate multiple versions toggle */}
                  <div className="flex items-center gap-3 py-2 border-t border-dashed border-gray-100">
                    ...
                    {email.abEnabled && <span className="text-[10px] text-violet-500 font-semibold">Leads are spread evenly across every version below — not identical wording to everyone</span>}
                  </div>

                  {/* Version 1 (the main subject/body editor above) */}
                  {email.abEnabled && <p className="text-xs font-bold text-violet-700 bg-violet-50 border border-violet-100 rounded-lg px-3 py-1.5">Version 1</p>}
                  ...
                  {/* Additional versions — repeatable list, not a single fixed slot */}
                  {email.abEnabled && email.abVariants.map((variant, vIdx) => (
                    <div key={vIdx} className="border border-violet-100 rounded-xl p-4 bg-violet-50/30 space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-xs font-bold text-violet-700">Version {vIdx + 2}</p>
                        <button type="button"
                          onClick={() => updateEmail(idx, 'abVariants', email.abVariants.filter((_, i) => i !== vIdx))}
                      ...
                      {(idx === 0 || email.threadMode === 'new_thread') && (
                        <input
                          placeholder={`Subject line for Version ${vIdx + 2}`}
                          value={variant.subject}
                          onChange={e => updateEmail(idx, 'abVariants', email.abVariants.map((v, i) => i === vIdx ? { ...v, subject: e.target.value } : v))}
                      ...
                      <textarea rows={6}
                        ...
                        value={variant.body}
                        onChange={e => updateEmail(idx, 'abVariants', email.abVariants.map((v, i) => i === vIdx ? { ...v, body: e.target.value } : v))}
                  ...
                  {email.abEnabled && (
                    <button type="button"
                      onClick={() => updateEmail(idx, 'abVariants', [...email.abVariants, { subject: '', body: '' }])}
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-dashed border-gray-100">
                    ...
                    <Toggle on={email.includeTracking} onToggle={() => updateEmail(idx, 'includeTracking', !email.includeTracking)}/>
                  </div>

                  <DeliverabilityCheckPanel subject={email.subject} body={email.body} includeTracking={email.includeTracking} senderAccounts={selectedAccountData} />
                </div>
              </div>
            ))}
                ...
                try {
                  ...
                  const res = await fetch('/api/campaigns', {
                    ...
                    body: JSON.stringify({
                      ...
                      steps: emails.map(e => ({ subject: e.subject, body: e.body, delay: e.delay, includeUnsub: e.includeUnsub, includeTracking: e.includeTracking, templateId: e.templateId, threadMode: e.threadMode, abVariants: e.abEnabled ? e.abVariants.filter(v => v.body?.trim()) : [] })),
