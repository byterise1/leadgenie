...
function Avatar({ name, isLead }: { name: string; isLead: boolean }) {
  ...
  return (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-[11px] font-bold ${isLead ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}>
      {initials}
    </div>
  );
}
...
export default function InboxPage() {
  ...
  return (
    <main className="flex-1 flex overflow-hidden" style={{ height: 'calc(100vh - 56px)' }}>
      ...
      <div className={`${selected ? 'hidden sm:flex' : 'flex'} w-full sm:w-80 shrink-0 border-r border-gray-100 flex-col bg-white`}>
        ...
        <div className="px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-800 space-y-3">
          <div className="flex items-center justify-between">
            <h1 className="text-base font-bold text-gray-900 dark:text-white">Unibox</h1>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => syncInbox(false)} disabled={syncing}
                className="flex items-center gap-1 text-[10px] font-bold text-gray-500 dark:text-gray-500 hover:text-blue-600 disabled:opacity-40 transition-colors">
          ...
          <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500">
            <span><span className="font-bold text-gray-700 dark:text-gray-200">{threads.length}</span> total</span>
            <span className="w-px h-3 bg-gray-200"/>
            <span><span className="font-bold text-gray-700 dark:text-gray-200">{unread}</span> unread</span>
            <span className="w-px h-3 bg-gray-200"/>
            <span><span className="font-bold text-gray-700 dark:text-gray-200">{filtered.length}</span> shown</span>
          </div>
          {/* Search */}
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, email, subject…"
              className="w-full pl-9 pr-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-xs outline-none focus:ring-2 focus:ring-blue-500 transition"/>
          </div>
          {/* Filters row */}
          <div className="flex gap-2">
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
              className="flex-1 border border-gray-200 dark:border-gray-700 rounded-xl bg-white px-2.5 py-1.5 text-[11px] font-semibold text-gray-700 dark:text-gray-200 outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer">
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-8 text-center h-full">
              ...
              <p className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">{threads.length > 0 ? 'No matches' : 'No replies yet'}</p>
            ...
            return (
              <button key={thread.id} onClick={() => selectThread(thread)}
                className={`w-full text-left px-4 py-3.5 border-b border-gray-50 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors ${selected?.id === thread.id ? 'bg-blue-50 border-l-2 border-l-blue-500' : ''}`}>
                <div className="flex items-start justify-between gap-2 mb-0.5">
                  ...
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 shrink-0">{relTime(thread.received_at)}</p>
                </div>
                <p className="text-xs text-gray-600 dark:text-gray-300 truncate">{thread.subject}</p>
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  {thread.lead?.company && (
                    <span className="text-[10px] text-gray-400 dark:text-gray-500 truncate max-w-[100px]">{thread.lead.company}</span>
                  )}
                  {statusDot && (
                ...
                {thread.last_message && (
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 truncate mt-0.5">{thread.last_message}</p>
                )}
              </button>
            );
          })}
      ...
      {selected ? (
        <div className="flex flex-1 flex-col bg-gray-50 dark:bg-gray-800 overflow-hidden">
          {/* Thread header */}
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-white shrink-0">
            {/* Mobile back button */}
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1">
                  <span className="text-xs text-gray-500 dark:text-gray-500 dark:text-gray-500">
                  ...
                  {selected.account?.email && (
                    <span className="text-[10px] text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 rounded px-1.5 py-0.5">via {selected.account.email}</span>
                  )}
                  {selected.campaign && (
                ...
                {[
                  ...
                  return (
                    <button key={s.key} onClick={() => updateStatus(selected.id, active ? 'new' : s.key)}
                      className={`text-[11px] font-bold px-2.5 py-1.5 rounded-lg border transition-all ${active ? meta.color : 'border-gray-200 text-gray-500 dark:text-gray-500 hover:border-gray-400 hover:text-gray-700'}`}>
                      {s.label}
                    </button>
                  );
                })}
                <button onClick={() => deleteThread(selected.id)} title="Delete thread"
                  className="p-1.5 rounded-lg border border-gray-200 text-gray-400 dark:text-gray-500 hover:border-red-300 hover:text-red-500 transition-all">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-2.5">
            {messagesLoading ? (
              <div className="space-y-3">
                {[1, 2].map(i => (
                  <div key={i} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 space-y-2">
                ...
                <div key={i} className={`rounded-xl border transition-all ${isReplyMsg ? 'bg-white border-blue-100 shadow-sm' : 'bg-white border-gray-100'}`}>
                  <button type="button" onClick={() => setExpandedSteps(prev => {
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-xs font-semibold text-gray-900 dark:text-white truncate max-w-[160px]">{senderName}</span>
                          ...
                          {msg.type === 'sent' && msg.step_number === 0 && <span className="shrink-0 text-[10px] text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800 border border-gray-100 rounded-full px-2 py-0.5">Email 1</span>}
                          {msg.type === 'sent' && msg.step_number > 0 && (
                            <span className="shrink-0 text-[10px] text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800 border border-gray-100 rounded-full px-2 py-0.5">
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-2">
                      ...
                      <svg className={`w-3.5 h-3.5 text-gray-400 dark:text-gray-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="px-4 pb-4 border-t border-gray-50">
                      {msg.subject && !msg.is_reply_thread && (
                        <div className="pt-3 pb-2">
                          <span className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide">Subject</span>
          ...
          {/* Reply composer */}
          <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-white shrink-0">
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all">
              <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-800 flex items-center gap-2">
                ...
                <span className="text-[11px] text-gray-500 dark:text-gray-500 truncate">
                  Reply to <span className="font-semibold text-gray-700 dark:text-gray-200">{selected.from_name || selected.from_email || selected.lead?.email}</span>
                  {selected.account?.email && <> from <span className="font-semibold text-gray-700 dark:text-gray-200">{selected.account.email}</span></>}
                </span>
              </div>
              <textarea
                value={replyBody}
              ...
              />
              <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <span className="text-[10px] text-gray-400 dark:text-gray-500">Ctrl+Enter to send</span>
        ...
        <div className="hidden sm:flex flex-1 flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 p-8 text-center">
          ...
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">Select a conversation</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 max-w-xs leading-relaxed">Replies from all your campaigns appear here in one unified view.</p>
        </div>
      )}
    </main>
  );
