    ...
    <tr>
      {Array.from({ length: 9 }).map((_, i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-3.5 bg-gray-100 dark:bg-gray-800 rounded animate-pulse w-full max-w-[80px]"/>
        </td>
      ))}
    </tr>
  );
    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="px-6 py-5">
          ...
          <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">{enabling ? 'Grant admin access?' : 'Remove admin access?'}</h3>
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800 flex gap-3 justify-end">
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 transition-colors">Cancel</button>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
        <div className="px-6 py-5 space-y-5">
          <div>
            <label className="text-xs font-bold text-gray-700 dark:text-gray-200 mb-2 block">Plan</label>
              ...
              {PLAN_OPTS.map(p => (
                <button key={p} type="button" onClick={() => handlePlanChange(p)}
                  className={`py-2 rounded-xl text-xs font-bold border transition-all capitalize ${
                    plan === p ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-500 border-gray-200 hover:border-gray-300'
                  }`}>
                  {p}
                </button>
              ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-700 dark:text-gray-200 mb-1.5 block">Credits Total</label>
              <input type="number" min={0} value={creditsTotal} onChange={e => setCreditsTotal(Number(e.target.value))}
                className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-700 dark:text-gray-200 mb-1.5 block">Credits Used</label>
          ...
          <div className={`flex items-center justify-between rounded-xl border px-4 py-3 ${isLastAdmin && isAdmin ? 'border-amber-200 bg-amber-50' : 'border-gray-200'}`}>
            <div>
              ...
              {isLastAdmin && isAdmin
                ...
                : <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Grant full admin panel access</p>
              }
            </div>
            <button
              type="button"
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800 flex gap-3 justify-end">
          <button onClick={onClose} className="px-5 py-2 text-sm font-semibold text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 transition-colors">Cancel</button>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between sticky top-0 bg-white z-10">
          <h2 className="text-base font-bold text-gray-900 dark:text-white">User Detail</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
        {loading ? (
          <div className="p-6 space-y-4">
            {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-10 bg-gray-100 dark:bg-gray-800 rounded-xl animate-pulse"/>)}
          </div>
        ) : !detail ? (
          <div className="p-6 text-center text-sm text-gray-400 dark:text-gray-500">Failed to load user details.</div>
        ) : (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                ...
                <div key={s.label} className="bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3">
                  <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase">{s.label}</p>
            ...
            {detail.accounts.length > 0 && (
              <div>
                <h3 className="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider mb-3">Email Accounts ({detail.accounts.length})</h3>
                <div className="space-y-2">
                  {detail.accounts.map(acc => (
                    <div key={acc.id} className="flex items-center gap-3 px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{acc.email}</p>
                        <p className="text-[10px] text-gray-400 dark:text-gray-500">{acc.type}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold rounded-full px-2 py-0.5 ${acc.status === 'active' ? 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
                          {acc.status}
                        </span>
                        <span className="text-[10px] text-gray-400 dark:text-gray-500">Score: {acc.health_score}</span>
                        {acc.warmup_enabled && <span className="text-[10px] bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-400 rounded-full px-2 py-0.5 font-bold">Warming</span>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {detail.campaigns.length > 0 && (
              <div>
                <h3 className="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider mb-3">Campaigns ({detail.campaigns.length})</h3>
                <div className="space-y-2">
                  {detail.campaigns.map(c => (
                    <div key={c.id} className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                      <p className="text-sm font-semibold text-gray-900 dark:text-white truncate flex-1 mr-4">{c.name}</p>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs text-gray-400 dark:text-gray-500">{c.total_sent} sent</span>
    ...
    <main className="flex-1 p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">{total.toLocaleString()} total accounts</p>
        </div>
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl border border-gray-100 p-5 border-t-2 border-t-blue-500">
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 mb-3">Total Users</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats ? adminStats.totalUsers.toLocaleString() : total.toLocaleString()}</p>
          <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-1">All-time accounts</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-5 border-t-2 border-t-emerald-500">
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 mb-3">New This Week</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats?.newUsersWeek ?? '—'}</p>
          <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-1">Last 7 days</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-5 border-t-2 border-t-violet-500">
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 mb-3">Emails Sent</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats ? adminStats.totalEmailsSent.toLocaleString() : '—'}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 border-t-2 border-t-amber-500">
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 mb-3">Active Campaigns</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats?.activeCampaigns ?? '—'}</p>
          ...
          <table className="w-full text-sm min-w-[820px]">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800">
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">User</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Plan</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Credits</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Sent</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Bounced</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Unsub</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Campaigns</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Joined</th>
                <th className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider whitespace-nowrap">Actions</th>
              </tr>
            </thead>
                ...
                users.map(u => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5">
                              <p className="text-sm font-semibold text-gray-900 dark:text-white truncate max-w-[140px]">{u.full_name || '—'}</p>
                              {u.is_admin && <span className="text-[9px] font-bold bg-red-100 text-red-600 rounded px-1">ADMIN</span>}
                            </div>
                            <p className="text-[11px] text-gray-400 dark:text-gray-500 truncate max-w-[160px]">{u.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{u.emails_sent}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-sm font-semibold ${u.bounced > 0 ? 'text-red-500' : 'text-gray-400'}`}>{u.bounced}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm font-semibold text-gray-500 dark:text-gray-500 dark:text-gray-500">{u.unsubscribed}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{u.campaigns}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-xs text-gray-500 dark:text-gray-500 dark:text-gray-500">{new Date(u.created_at).toLocaleDateString()}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => setDetailId(u.id)}
                            className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="View details">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                          </button>
                          <button onClick={() => setEditUser(u)}
                            className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Edit user">
          ...
          <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className="px-3 py-1.5 text-xs font-semibold rounded-lg border border-gray-200 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-800 disabled:opacity-40 transition-colors">
                Previous
              </button>
...
export default function AdminUsersPage() {
  return (
    <Suspense fallback={<main className="flex-1 p-6"><div className="h-8 w-48 bg-gray-100 dark:bg-gray-800 rounded animate-pulse"/></main>}>
      <AdminUsersInner />
    </Suspense>
  );
}
