...
- **What was actually built (4 real code changes, all additive — every existing `notifications` table insert kept exactly as-is, email is a NEW side effect alongside it, never a replacement):**
  1. `lib/resend.ts` (new) — `sendTransactionalEmail()` (thin Resend SDK wrapper, branded HTML shell, "Leads Add" from-name, fails silently/logged-only so a Resend hiccup never breaks the calling code path) and `notifyUserByEmail()` (looks up the user's real Supabase Auth email — not the profile table — and sends via the above).
