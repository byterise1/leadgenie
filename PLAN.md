...
**IMPLEMENTED 2026-07-07 ("go ahead"):** confirmed zero active campaigns in production before starting (no live traffic disrupted). New: `lib/campaign-scheduling.ts` (shared window helpers + priority engine pure functions, unit-verified including the exact 6000/250 scenario), a new recurring campaign-scheduler worker in `instrumentation.ts` (every 2 min, same pattern as the warmup worker) that replaced the email-sending worker's old self-chaining/pre-scheduled-delay model, and a "Follow-up Priority" Auto/Manual control on the campaign detail page. Caught one gap mid-build not in the original plan: `campaign_leads` needed a new `account_id` column to durably persist the locked mailbox per lead, since that continuity previously only existed as data passed job-to-job in the now-removed queue chain. Full detail in `[memory] project-campaign-sending.md` under "IMPLEMENTED 2026-07-07". **Action needed: user must run `supabase/migrations/20260707_smart_priority_engine.sql` in the Supabase SQL Editor before this is live** (no direct DB connection available from this environment to run it automatically).
