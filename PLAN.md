...
**⚙️ There IS a live per-campaign test system now (2026-07-10, separate from the above, still active) — read this if resuming:** `TEST_MODE_FAST_FOLLOWUPS` (`lib/campaign-scheduling.ts`) is currently `true`. This does NOT affect the global constant above or any existing campaign — it only means any NEW campaign created right now gets stamped with a 1-real-minute-per-"day" follow-up delay instead of 24h, permanently, for that one campaign (see the big comment above the flag in the code, and `[memory] project-campaign-sending.md` for the full design). A companion "⏩ Skip to next day" button also exists on test-unit campaigns to click through a sequence without even waiting the 1 minute. **If the user says "switch back to production" about follow-up testing: set `TEST_MODE_FAST_FOLLOWUPS = false` in `lib/campaign-scheduling.ts`, commit, push — that's the entire revert, nothing else changes.** Campaigns already created while it was `true` keep their fast timing forever, by design.

**Migrations — all run, none pending, as of 2026-07-10:** `supabase/migrations/20260710_pacing_and_auto_limit.sql` (`email_accounts.next_dispatch_at`, `campaigns.daily_limit_mode`) and `supabase/migrations/20260710_step_delay_unit.sql` (`campaigns.step_delay_unit_ms`) have both been run by the user and confirmed live. All earlier migrations (`20260705_warmup_phase1.sql`, `20260707_smart_priority_engine.sql`, `20260709_step_id_tracking.sql`) remain run and verified live too. There is no automated runner — Claude cannot execute DDL directly against this database (no direct Postgres connection available, only the Supabase JS client).
