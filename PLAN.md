...
**Migrations — 1 pending as of 2026-07-11:** `supabase/migrations/20260711_test_campaign_flag.sql` (adds `campaigns.is_test_campaign`, backfills existing fast-mode campaigns to `true`) is written but **NOT YET RUN — user must run it in the Supabase SQL Editor before deploying this code**, otherwise `POST /api/campaigns` will error on every new campaign (inserting into a column that doesn't exist yet). All earlier migrations (`20260705_warmup_phase1.sql` through `20260710_step_delay_unit.sql`) remain run and verified live. There is no automated runner — Claude cannot execute DDL directly against this database (no direct Postgres connection available, only the Supabase JS client, which can read/write rows but not ALTER TABLE).
