...
- **Still open, needs a decision, not touched:** does editing a step's delay on a PAUSED campaign retroactively recompute `next_send_at` for leads already waiting on that exact step (computed under the OLD delay value), or only affect leads that reach that step in the future? Currently: only future leads — an already-scheduled lead's due time stays whatever it was originally computed as when edited. User's phrasing ("if change then if one day it should be replace") suggests they may want the former (retroactive recompute) — this touches existing working step-edit behavior, so asking rather than assuming before changing it.
