...
**Migrations — ONE PENDING as of 2026-07-10:** `supabase/migrations/20260710_pacing_and_auto_limit.sql` has been written and committed but **NOT YET RUN** — must be pasted into the Supabase SQL Editor and executed before the per-mailbox pacing fix and Auto daily-limit mode take effect in production. It's a 2-column additive migration (`email_accounts.next_dispatch_at`, `campaigns.daily_limit_mode`), fully safe to run any time — the code fails open (Supabase-js doesn't throw on a missing-column error, it just logs and no-ops) so nothing breaks before it's run, the pacing bug just isn't fixed yet. All earlier migrations (`20260705_warmup_phase1.sql`, `20260707_smart_priority_engine.sql`, `20260709_step_id_tracking.sql`) remain run and verified live. There is no automated runner — Claude cannot execute DDL directly against this database (no direct Postgres connection available, only the Supabase JS client).
...
- **⚠️ ACTION NEEDED FROM USER:** run `supabase/migrations/20260710_pacing_and_auto_limit.sql` in the Supabase SQL Editor — see the Migrations note above. The pacing fix and Auto daily-limit mode silently no-op (fail safely, nothing breaks) until this is run.
